  ...
  return (
    <div className="w-full min-h-full box-border">
        ...
        <div className="fixed inset-0 bg-black/35 backdrop-blur-sm flex items-center justify-center z-[9999]">
          <div className={`w-[min(430px,90%)] p-[22px] rounded-3xl bg-[linear-gradient(135deg,rgba(12,10,30,0.98),rgba(6,7,15,0.98))] shadow-[0_24px_70px_rgba(0,0,0,0.55)] flex items-center gap-[15px] text-white border ${isError ? "border-red-500/[0.40]" : "border-emerald-500/[0.40]"}`}>
            ...
            <div>
              ...
              <p className="m-0 text-sm leading-[1.5] text-white/60">{messageBox.message}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-3 mb-6">
        <div className="w-[52px] h-[52px] rounded-2xl bg-emerald-500/[0.12] border border-emerald-500/[0.22] flex items-center justify-center text-emerald-400">
          <MdDone size={26} />
        </div>
        <div>
          <h2 className="m-0 text-[28px] font-extrabold text-white">Done Tasks</h2>
          <p className="m-0 text-white/50 text-sm">All completed tasks.</p>
        </div>
      </div>

      {notasks && (
        <div className="flex flex-col items-center justify-center py-16 px-5 text-center">
          <div className="w-[72px] h-[72px] rounded-2xl bg-emerald-500/[0.10] border border-emerald-500/[0.20] flex items-center justify-center mb-5">
            <MdDone size={36} className="text-emerald-400" />
          </div>
          <h2 className="m-0 mb-2 text-[24px] font-extrabold text-white">No completed tasks</h2>
          <p className="m-0 text-white/45 text-[15px] max-w-[320px] leading-[1.7]">You haven't completed any tasks yet. Check off a task and it'll show up here.</p>
        </div>
      )}
