...
const SignUp = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (data.password !== data.repassword) {
      setMatching(false);
      if (passwordRef.current) passwordRef.current.style.border = "2px solid rgba(239,68,68,0.6)";
      if (repasswordRef.current) repasswordRef.current.style.border = "2px solid rgba(239,68,68,0.6)";
      return;
    }
  ...
  return (
    <>
      ...
      <main className="w-full min-h-[calc(100vh-70px)] flex justify-center items-center px-5 py-[30px] box-border bg-[#06070f]">
        <div
          className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[600px] rounded-full pointer-events-none"
          style={{ background: "radial-gradient(circle, rgba(99,102,241,0.07), transparent 60%)" }}
        />
        <div className="w-full max-w-[460px] p-[38px_32px] rounded-[28px] bg-white/[0.05] border border-indigo-500/[0.18] shadow-[0_22px_60px_rgba(0,0,0,0.45)] backdrop-blur-[14px] box-border relative z-10">
          <h2 className="m-0 text-white text-[32px] font-extrabold text-center tracking-[0.4px]">Create Account</h2>
          <p className="mt-2.5 mb-7 text-white/55 text-center text-[15px] leading-[1.7]">
          ...
          <form onSubmit={handleSubmit}>
            <div className="flex flex-col gap-2 mb-[18px]">
              <label className="text-indigo-200 text-sm font-bold tracking-[0.3px]">Email</label>
            ...
            <div className="flex flex-col gap-2 mb-[18px]">
              <label className="text-indigo-200 text-sm font-bold tracking-[0.3px]">Username</label>
            ...
            <div className="flex flex-col gap-2 mb-[18px]">
              <label className="text-indigo-200 text-sm font-bold tracking-[0.3px]">Password</label>
            ...
            <div className="flex flex-col gap-2 mb-[18px]">
              <label className="text-indigo-200 text-sm font-bold tracking-[0.3px]">Confirm Password</label>
            ...
            {!matching && (
              <div className="flex items-center gap-3 mb-4 px-4 py-3 rounded-[12px] border border-red-500/[0.35] bg-red-500/[0.09]">
                <MdErrorOutline size={20} className="text-red-400 shrink-0" />
                <p className="m-0 text-red-300 text-sm font-semibold">Password and Confirm Password must match</p>
              </div>
            )}
            <button type="submit" className="w-full h-[50px] rounded-[14px] border-none bg-gradient-to-r from-indigo-600 to-violet-600 text-white text-base font-extrabold cursor-pointer shadow-[0_12px_28px_rgba(99,102,241,0.28)] transition-all duration-300 hover:-translate-y-0.5 mt-2.5">
              Sign Up
            </button>
            <div className="mt-3 text-center text-white/55 text-sm">
              Already have an account?<Link to="/login" className="text-indigo-400 no-underline font-bold ml-1.5 hover:text-indigo-300 transition-colors">Login</Link>
            </div>
          </form>
        </div>
      </main>
